...
from benchmarks.workloads.cifar10.resnet18_groupnorm import tensor_schema_for_model
...
def test_dromeus_adapter_projects_one_exact_noloco_draft(tmp_path: Path) -> None:
    ...
    assert draft.require_iid_dataset().partition_sample_counts == (6_250,) * 8
    assert draft.require_iid_dataset().node_index_partitions == tuple(range(8))
