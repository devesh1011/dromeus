...
from benchmarks.workloads.cifar10.partitioning import ClassificationData, DataProvenance
...
class _HuggingFaceCIFAR10(Dataset[tuple[Tensor, int]]):
    ...
    def __getitem__(self, index: int) -> tuple[Tensor, int]:
        ...
        if pixels.shape != (IMAGE_SHAPE[1], IMAGE_SHAPE[2], IMAGE_SHAPE[0]):
            raise CIFAR10DataError(f"expected CIFAR-10 image shape, got {pixels.shape}")
        image = (
...
def _prepare_batch(
    ...
    *,
    ...
    if augment:
        flips = (torch.rand(images.shape[0], generator=generator) < 0.5).to(
            images.device
        )
        if bool(flips.any()):
            images = images.clone()
...
class PreparedCIFAR10Training:
    ...
    def create_trainer(
        ...
        *,
        ...
        partition_index = manifest.require_iid_dataset().node_index_partitions[
            node_index
        ]
...
def prepare_training(
    *,
    ...
    if len(train_data) != dataset.sample_count:
    ...
    if tuple(len(partition) for partition in partitions) != (
        dataset.partition_sample_counts
    ):
