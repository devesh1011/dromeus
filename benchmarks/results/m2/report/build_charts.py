...
plt.rcParams.update(
    {
        ...
        "axes.titlecolor": "#285477",
        "axes.labelcolor": "#263444",
...
def save(fig: Figure, name: str) -> None:
    # Matplotlib's keyword arguments have incomplete stubs. Match the scoped
    # call-site annotations used by benchmarks/cifar10/plots.py.
    fig.savefig(  # pyright: ignore[reportUnknownMemberType]
        OUT / name, dpi=200, bbox_inches="tight"
    )
    plt.close(fig)
...
for row, w in enumerate(SCALES):
    for seed, color in zip(SEEDS, COLORS):
        ...
        assert set(metrics) == set(range(500)) and all(
            len(v) == w for v in metrics.values()
        )
        evaluation = [
            r
            for r in sorted(metrics)
            if all("evaluation_accuracy" in v for v in metrics[r])
        ]
        acc = [
            np.mean([v["evaluation_accuracy"] for v in metrics[r]]) * 100
            for r in evaluation
        ]
        loss = [np.mean([v["local_loss"] for v in metrics[r]]) for r in sorted(metrics)]
        axes[row, 0].plot(
            np.array(evaluation) + 1, acc, color=color, label=f"Seed {seed}", lw=1.5
        )
        axes[row, 1].plot(
            range(1, 501), loss, color=color, label=f"Seed {seed}", lw=1.2, alpha=0.85
        )
        timings = {
            k: float(np.mean([r[k] for r in rec["rounds"]]))
            for k in [
        ...
        runs[f"{w}-{seed}"] = {
            ...
            "curves": {
                "evaluation_outer_rounds": [r + 1 for r in evaluation],
...
axes[-1, 0].set_xlabel("Completed outer rounds")
axes[-1, 1].set_xlabel("Completed outer rounds")
...
for ax, w in zip(axes, SCALES):
    for seed, color in zip(SEEDS, COLORS):
        ...
        ax.plot(
            [p["completed_outer_steps"] for p in points],
            [p["weight_std_l2"] for p in points],
            color=color,
            label=f"Seed {seed}",
            lw=1.5,
        )
    ax.set_title(f"{w} workers")
    ax.set_xlabel("Outer rounds")
    ax.grid(alpha=0.16)
axes[0].set_ylabel("Weight standard deviation (L2)")
axes[0].legend(fontsize=7)
fig.tight_layout()
save(fig, "divergence.png")

identity_report = read(
    ROOT / "identity-ablation/4-nodes/reports/acceptance-report.json"
)
...
for i, v in enumerate([identity_wire, compressed_wire]):
...
for col, (name, color) in enumerate(
    zip(
        ["Local compute", "Peer wait", "Transfer", "Outer update", "Evaluation"],
        ["#3274A5", "#A9BDCC", "#4F9380", "#D58A35", "#856AA2"],
    )
):
    vals = np.array(data)[:, col]
    ax.bar(  # pyright: ignore[reportUnknownMemberType]
        range(9), vals, bottom=bottom, label=name, color=color
    )
    bottom += vals
ax.set_xticks(range(9), labels)  # pyright: ignore[reportUnknownMemberType]
ax.set_ylabel(  # pyright: ignore[reportUnknownMemberType]
    "Mean seconds / node-round"
)
ax.set_title(  # pyright: ignore[reportUnknownMemberType]
    "AXL recorded round-time components"
)
...
summary = {
    "runs": runs,
    "identity_ablation": {
        "mean_accuracy_percent": identity_accuracy,
        "source": "identity-ablation/4-nodes/reports/acceptance-report.json",
    },
    "sources_sha256": sources,
    "notes": {
        "learning": (
            "Worker means; evaluation at retained cadence; "
            "loss is unsmoothed final local batch loss per round."
        ),
        "timing": (
            "Means of recorded per-node timing fields; "
            "not a synchronized global wall-clock measurement."
        ),
    },
}
...
for w in SCALES:
    print(
        ...
        w,
        "mean accuracy",
        np.mean([runs[f"{w}-{s}"]["accuracy"]["mean"] for s in SEEDS]) * 100,
        "lifecycle minutes",
        [round(runs[f"{w}-{s}"]["lifecycle_seconds"] / 60, 2) for s in SEEDS],
        "mean round seconds",
        [round(sum(runs[f"{w}-{s}"]["timings"].values()), 2) for s in SEEDS],
        "residual maxima",
        max(runs[f"{w}-{s}"]["residual"]["maximum_l2_norm"] for s in SEEDS),
        "ratio maxima",
        max(runs[f"{w}-{s}"]["residual"]["maximum_to_signal_ratio"] for s in SEEDS),
    )
