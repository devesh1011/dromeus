...
sudo docker run --rm --gpus all --network host --shm-size 1g --env PYTHONUNBUFFERED=1 --volume "${base_root}:${base_root}" "$image" .venv/bin/python -m benchmarks.noloco.dromeus_official_runner --node-config "${runtime_root}/node-${rank}.yaml" --experiment "${base_root}/frozen-topk45-official/experiment.yaml" --world-size 4 --seed 17 --output-dir "${runtime_root}/analysis/rank-${rank}"
status=$?
