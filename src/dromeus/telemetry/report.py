...
class ExactConsensusReport:
    ...
    def render_svg(self, *, width: int = 900, height: int = 480) -> str:
        """Render pre-mix, post-mix, and smoothed post-mix curves as SVG."""
        ...
        values = [
            ...
            for value in (
                round_report.pre_mix_distance,
                round_report.post_mix_distance,
                round_report.smoothed_post_mix_distance,
            )
        ]
        ...
        def point(index: int, value: float) -> tuple[float, float]:
            x = margin_left + (
                ...
                else plot_width * index / (len(self.rounds) - 1)
            )
            y = margin_top + plot_height * (1 - value / maximum)
            return x, y

        def path(values_for_rounds: Sequence[float]) -> str:
            commands: list[str] = []
            for index, value in enumerate(values_for_rounds):
                x, y = point(index, value)
                commands.append(f"{'M' if index == 0 else 'L'} {x:.2f},{y:.2f}")
            return " ".join(commands)

        pre = [round_report.pre_mix_distance for round_report in self.rounds]
        post = [round_report.post_mix_distance for round_report in self.rounds]
        smoothed = [
            round_report.smoothed_post_mix_distance for round_report in self.rounds
        ]
        lines = [
            (
            ...
            (
                f'<text x="{margin_left}" y="20" '
                'font-family="sans-serif" font-size="16">'
                'Exact consensus distance</text>'
            ),
            (
                f'<text x="{margin_left}" y="{height - 18}" '
                'font-family="sans-serif" font-size="12">round</text>'
            ),
            (
                f'<text x="15" y="{margin_top + plot_height / 2}" '
                f'transform="rotate(-90 15 {margin_top + plot_height / 2})" '
                'font-family="sans-serif" font-size="12">normalized RMS</text>'
            ),
            (
                f'<path d="{path(pre)}" fill="none" '
                'stroke="#2563eb" stroke-width="2"/>'
            ),
            (
                f'<path d="{path(post)}" fill="none" '
                'stroke="#dc2626" stroke-width="2"/>'
            ),
            (
                f'<path d="{path(smoothed)}" fill="none" '
                'stroke="#16a34a" stroke-width="2"/>'
            ),
            (
                f'<text x="{width - 260}" y="25" '
                'font-family="sans-serif" font-size="12" '
                'fill="#2563eb">pre-mix</text>'
            ),
            (
                f'<text x="{width - 190}" y="25" '
                'font-family="sans-serif" font-size="12" '
                'fill="#dc2626">post-mix</text>'
            ),
            (
                f'<text x="{width - 105}" y="25" '
                'font-family="sans-serif" font-size="12" '
                'fill="#16a34a">smoothed</text>'
            ),
            "</svg>",
        ]
        return "\n".join(lines)
    ...
    def write_svg(self, path: Path) -> None:
        """Write the exact consensus curves as a self-contained SVG."""
        path.parent.mkdir(parents=True, exist_ok=True)
